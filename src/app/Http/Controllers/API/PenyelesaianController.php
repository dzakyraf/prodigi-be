...
class PenyelesaianController extends AppBaseController
{
    ...
    public function saveDraft(SaveDraftRendanAPIRequest $request): JsonResponse
    {
        ...
        $file_ms  = $this->proposalModel->getDocuments(4, $procurement_id);

        $update = [];
        if (isset($input['tagihan_amount']))
            $update['tagihan_amount'] = $input['tagihan_amount'];
        if (isset($input['penilaian_pekerjaan']))
            $update['penilaian_pekerjaan'] = $input['penilaian_pekerjaan'];
        if (isset($input['tkdn_percent']))
            $update['tkdn_percent'] = $input['tkdn_percent'];
        ...
        foreach ($file_ms as $doc) {
            ...
            if ($value != null) {
                $form  = ProcurementDocsData::from([
                    'procurement_process_id' => ProcurementProcess::DOKUMEN_PENYELESAIAN->value,
    ...
    public function submit(SaveDraftRendanAPIRequest $request): JsonResponse
    {
        ...
        DB::table('prod_procurement')
            ->where('procurement_id', $procurement_id)
            ->update(['last_status' => ApprovalStatus::NeedReview->value, 'last_process_id' => ProcurementProcess::DOKUMEN_PENYELESAIAN->value,]);
